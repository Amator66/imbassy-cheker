...
if (!TOKEN) {
  console.error('TELEGRAM_BOT_TOKEN در Environment Variables پیدا نشد.');
  process.exit(1);
}
...
async function sendTelegram(message) {
  if (!chatId) {
    console.log('هنوز Chat ID نداریم. در تلگرام /start بفرست.');
    return;
  }
...
async function selectFirstRealOption(select) {
  ...
  try {
    ...
    console.log(
      'انتخاب گزینه ممکن نبود: ' + error.message
    );
...
async function checkAppointments(notify = true) {
  ...
  try {
    ...
    console.log(
      'Selectهای قابل مشاهده: ' + selectCount
    );
    ...
    if (nextCount > 0) {
      ...
      console.log(
        'وارد مرحله Appointment Date & Time شدیم.'
      );
...
bot.onText(/^\/start$/, async msg => {
  ...
  await bot.sendMessage(
    ...
    'ربات آماده است.\n\n' +
      ...
      'هر پنج دقیقه سایت سفارت را بررسی می‌کنم.\n\n' +
      'برای بررسی دستی:\n' +
      '/check'
  );
});
